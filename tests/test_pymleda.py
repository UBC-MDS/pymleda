...
def test_dftype():
    ...
    assert type(pymleda.dftype(df)[0]) == (pd.core.frame.DataFrame), \
        "The date type of summry should be data frame."    
   
    assert type(pymleda.dftype(df)[1]) == (pd.core.frame.DataFrame), \
         "The date type of unique_val should be data frame."
    
    assert len(pymleda.dftype(df)[0]) == 8, \
         "The length of summary data frame is incorrect."
    
    assert list(pymleda.dftype(df)[1].unique_values[0]) == ['US', 'Mexico', 'CANADA', 'UK'], \
         "The unique values are incorrect."
